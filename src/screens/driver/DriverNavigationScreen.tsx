...
export const DriverNavigationScreen = ({ navigation, route }: Props) => {
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    destinationLabel: { color: tokens.text.secondary },
    ...
    centerButton: {
      alignSelf: 'flex-end', backgroundColor: tokens.card, width: 56, height: 56,
  ...
  if (
    ...
    return (
      <View style={styles.loadingContainer}>
        ...
        <ChocolateButton variant="outline" onPress={() => navigation.goBack()} style={{ marginTop: 16 }}>
          {t('common.back')}
        </ChocolateButton>
      </View>
    );
      ...
      <Mapbox.MapView style={styles.map}>
        ...
        <Mapbox.PointAnnotation id="pickup" coordinate={pickupCoord}>
          <View style={[styles.waypointMarker,
            { backgroundColor: isGoingToPickup ? colors.primary : tokens.border }]} />
          <Mapbox.Callout title={t('transport.loading_point')} />
        </Mapbox.PointAnnotation>
        <Mapbox.PointAnnotation id="delivery" coordinate={deliveryCoord}>
          <View style={[styles.waypointMarker,
            { backgroundColor: !isGoingToPickup ? colors.success : tokens.border }]} />
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.bottomActions}>
        ...
        <ChocolateButton onPress={openExternalNavigation} style={styles.externalNavButton}>
          {t('driver.open_in_google_maps')}
        </ChocolateButton>
        <ChocolateButton variant="outline" onPress={() => navigation.goBack()} style={styles.backButton}>
